...
namespace SchoolManagement
{
    public partial class VoucherApproval : System.Web.UI.Page
    {
        ...
        protected void LoadApprovalPendingVoucher()
        {
            try
            {
                Hashtable _param = new Hashtable();
                _param.Add("Action", "LoadPendingForApproval");

                _reader = new DataReader();
                DataTable dtVoucher = _reader.GetDataTableByStoredProcedure("SP_VOUCHER_ENTRY", _param);
                if (dtVoucher.Rows.Count > 0)
                {
                    lblRecordCount.Text = dtVoucher.Rows.Count.ToString();
        ...
        protected void btnApproved_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result.IsValid == true)
                {
                    ...
                    _param.Add("Remarks", txtDescription.Text.ToString());
                    _param.Add("EntryBy", user.UserID.ToString());
                    _param.Add("Action", "ApproveVoucher");

                    _writer = new DataWriter();
                    long iAffRow = _writer.ExecuteStoredProcedure("SP_VOUCHER_APPROVAL", _param, "AffRow");
                    if (iAffRow > 0)
                    {
                        ActivityLog.SaveProcess(ActionType.Approved, "SP_VOUCHER_APPROVAL", "ApproveVoucher", hdVoucherId.Value);
                        UnlockMasterControls();
                        ClearAllControls();
                        ClearAccountInfo();
                        LoadApprovalPendingVoucher();
                        DisplayMessage("Voucher has been approved successfully.");
                    }
                }
                else
                {
            ...
            {
                ErrorTracking.SaveError(user.UserID, sPageName, "btnApproved_Click", ex);
                DisplayMessage(ex.Message.ToString(), "Contact to Software Administrator");
            }
        }
        ...
        protected void btnDisapproved_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _param.Add("Remarks", txtDescription.Text.ToString());
                _param.Add("EntryBy", user.UserID.ToString());
                _param.Add("Action", "DisapproveVoucher");

                _writer = new DataWriter();
                long iAffRow = _writer.ExecuteStoredProcedure("SP_VOUCHER_APPROVAL", _param, "AffRow");
                if (iAffRow > 0)
                {
                    ActivityLog.SaveProcess(ActionType.Disapproved, "SP_VOUCHER_APPROVAL", "DisapproveVoucher", hdVoucherId.Value);
                    DisplayMessage("Voucher has been disapproved successfully.");
            ...
            {
                ErrorTracking.SaveError(user.UserID, sPageName, "btnDisapproved_Click", ex);
                DisplayMessage(ex.Message.ToString(), "Contact to Software Administrator");
            }
        }
